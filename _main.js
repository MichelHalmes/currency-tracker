...
const FINANCE_URL = 'https://api.exchangeratesapi.io/latest?symbols=ZAR'
const OPTIONS = {
  uri: FINANCE_URL, 
  json: true, 
  headers: {Accept: "application/json"}
}
var eur_zar_p = rp(OPTIONS)
  .catch(err => {
    console.log("An error occured, retrying once")
    return rp(OPTIONS)
  })
  .then(res => {
    console.log("+=")
    let eur_zar = res.rates.ZAR
...
const ALERT_EUR_ZAR_BELOW = 15.85
...
Promise.all([eur_zar_p]) //, otter_p])
  .then(results => {
    ...
    var channel = eur_zar < ALERT_EUR_ZAR_BELOW ? MICHEL_CHNL : LOG_CHNL
